import pandas as pd
import pytest
from src.business_logic.gps_processor import GpsProcessor


def test_to_dataframe_empty_list():
    result = GpsProcessor.to_dataframe([])
    assert result.empty


def test_to_dataframe_preserves_data():
    data = [{"lat": 31.5, "lon": 35.0}, {"lat": 31.6, "lon": 35.1}]
    result = GpsProcessor.to_dataframe(data)
    assert len(result) == 2
...
def test_format_for_display_mutates_original():
    ...
    GpsProcessor.format_for_display(df)
